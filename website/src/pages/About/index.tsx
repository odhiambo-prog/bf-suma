    ...
    <div className="min-h-screen bg-surface">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
            <Reveal>
              <motion.div
                ...
                viewport={{ once: true }}
                className="border border-surface-border bg-white/90 backdrop-blur-sm p-8 rounded-3xl"
              >
                <h3 className="font-display text-xl text-ink mb-4">Our Vision</h3>
            ...
            <Reveal delay={0.08}>
              <motion.div
                ...
                className="border border-surface-border bg-white/90 backdrop-blur-sm p-8 rounded-3xl"
              >
                <h3 className="font-display text-xl text-ink mb-4">Our Mission</h3>
                <p className="text-sm text-muted-500 leading-relaxed">
      ...
      <section className="py-16 md:py-28 bg-surface-subtle">
        <div className="max-w-7xl mx-auto px-6">
          <SectionIntro variant="withTag" tag="Our Values" title="What We Stand For" />
          <div className="mt-12 -mx-6 sm:mx-0 mb-12 overflow-hidden sm:rounded-3xl">
            <img
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((value, i) => (
              <Reveal key={i} delay={i * 0.05}>
                <FloatingSurface lift className="p-8 h-full">
                  <h3 className="text-sm font-semibold text-ink mb-2">{value.title}</h3>
                  <p className="text-xs text-muted-500 leading-relaxed">{value.desc}</p>
                </FloatingSurface>
              </Reveal>
            ))}
          </div>
      ...
      {team.length > 0 && (
        <section className="py-16 md:py-28 bg-white overflow-hidden">
          <div className="max-w-7xl mx-auto px-6">
            <SectionIntro title="Faces of Eagle Team" subtitle="The people behind BF SUMA Nairobi, committed to your wellness journey." />
          </div>
          <div className="relative mt-12">
            <div className="overflow-x-auto pb-4 hide-scrollbar cursor-grab active:cursor-grabbing flex touch-pan-x"
              onMouseDown={(e) => { const el = e.currentTarget; const startX = e.pageX - el.offsetLeft, scrollLeft = el.scrollLeft; const onMove = (ev: MouseEvent) => { ev.preventDefault(); el.scrollLeft = scrollLeft - (ev.pageX - startX); }; const onUp = () => { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); }; document.addEventListener('mousemove', onMove); document.addEventListener('mouseup', onUp); }}
            >
              <div className="flex gap-10 px-6 flex-shrink-0 mx-auto">
                {team.map((member) => (
                  ...
                  >
                    <div className="relative mx-auto w-24 h-24 md:w-28 md:h-28 rounded-full bg-gradient-to-br from-jade-400 to-ink flex items-center justify-center mb-4 shadow-lg group-hover:scale-105 transition-transform duration-300 overflow-hidden">
                    ...
                    <h3 className="text-sm font-semibold text-ink mb-0.5">{member.name}</h3>
                  </div>
                ))}
              </div>
            </div>
